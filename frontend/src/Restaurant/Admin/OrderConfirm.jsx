...
const OrderConfirm = () => {
  ...
  // Fetch orders for the authenticated restaurant, with status filter for "pending"
  const {
    data: ordersData,
    isLoading,
    refetch,
    ...
    {
      skip: !restaurantId, // Skip the query if we don't have a restaurant ID
    }
  );
  ...
  const getPaymentInfo = (payment) => {
    ...
    if (payment.method === "creditCard" || payment.method === "card") {
      return {
